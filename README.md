...
Language note: This README uses an English-first bilingual structure with one-to-one Chinese correspondence. The Chinese section is not a supplemental summary; it is an equivalent version of the English section. The skill itself is written in English because it is long-lived operational guidance for Codex.

语言说明：本 README 采用英文优先、中文一一对应的双语结构。中文部分不是补充摘要，而是英文部分的等价版本。Skill 本体使用英文，因为它是给 Codex 长期读取和执行的规则。
